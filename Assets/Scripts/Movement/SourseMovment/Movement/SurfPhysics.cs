...
namespace Movement.SourseMovment.Movement
{
    public class SurfPhysics
    {
        ...
        /// </summary>
        public static int
            groundLayerMask = LayerMask.GetMask("Default", "Ground", "Player clip"); //(1 << 0);
            ...
            ISurfControllable surfer = null)
        {
            ...
            for (var i = 0; i < numOverlaps; i++)
            {
                        ...
                        _colliders[i].transform.rotation, out direction, out distance))
                {
                    ...
                    var rb = _colliders[i].GetComponentInParent<Rigidbody>();
            ...
            Vector3 direction, float distance, Vector3 forwardVelocity, ISurfControllable surfer)
        {
            ...
            var forwardTrace = Tracer.TraceCollider(collider, upOrigin,
            ...
            var downTrace = Tracer.TraceCollider(collider, upForwardOrigin, upForwardOrigin + Vector3.down * upDistance,
                groundLayerMask);
