...
{
    [Header("References")] public Transform bodyTransform;

    [Header("Sensitivity")] public float sensitivityMultiplier = 1f;
    public float horizontalSensitivity = 1f;
    public float verticalSensitivity = 1f;

    [Header("Restrictions")] public float minYRotation = -90f;
    ...
    [HideInInspector] public Vector2 punchAngle;

    [HideInInspector] public Vector2 punchAngleVel;
    ...
    private void Update()
    {
        ...
        // Input
        var xMovement = Input.GetAxisRaw("Mouse X") * horizontalSensitivity * sensitivityMultiplier;
        var yMovement = -Input.GetAxisRaw("Mouse Y") * verticalSensitivity * sensitivityMultiplier;
    ...
    {
        var rotation = new Vector3(gameObject.transform.rotation.eulerAngles.x,
            gameObject.transform.rotation.eulerAngles.y, 0);

        if (gameObject.transform.rotation.eulerAngles.x >= 180)
            rotation.x = -180 + (gameObject.transform.rotation.eulerAngles.x - 180);
        if (gameObject.transform.rotation.eulerAngles.y >= 180)
            rotation.y = -180 + (gameObject.transform.rotation.eulerAngles.y - 180);
    ...
    private void DecayPunchAngle()
    {
        if (punchAngle.sqrMagnitude > 0.001 || punchAngleVel.sqrMagnitude > 0.001)
        {
            ...
            var damping = 1 - punchDamping * Time.deltaTime;
