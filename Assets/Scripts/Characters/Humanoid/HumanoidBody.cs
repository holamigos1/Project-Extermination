...
namespace Characters.Humanoid
{
	...
	{
		//TODO Поле жирновато, убери всё в структуры настроек тела..
		...
		private bool IsRotating => _rootRotationDelta == Quaternion.identity;
		...
		private void Reset()
		{
			...
			_humanAnimatorSheet = AssetDataBaseExtensions.LoadAssets<HumanAnimatorSheet>().First();

			if (_humanAnimatorSheet == null)
				Debug.LogError($"Создай список {nameof(HumanAnimatorSheet)} в проекте!"); //TODO А пусть программа сама создаст что ей надо
		}
	#endif
		...
		{
			if (_transform == null) 
				Debug.LogError($"Reset {nameof(HumanoidBody)}!");
			if (_gameObject == null) 
				Debug.LogError($"Reset {nameof(HumanoidBody)}!");
			if (_animator == null) 
				Debug.LogError($"Reset {nameof(HumanoidBody)}!");
		...
		public void ApplySprint(bool isSprinting) =>
			_humanoidBodyParameters.CurrentMovementType = isSprinting ?
				HumanoidBodyParameters.MovementType.Sprinting :
				HumanoidBodyParameters.MovementType.Walking;
		...
		public void ApplyAim(bool isAiming)
		{
			if (ItemInRightHand != null &&
				_weaponHandler.IsMatchToCurrentEquipedWeapon(ItemInRightHand) == false) 
				return;
		...
		{
			if ((ItemInRightHand != null) && (_weaponHandler.IsMatchToCurrentEquipedWeapon(ItemInRightHand))) 
		...
		public void ApplyPickUp(GameItem item) //TODO груда ифов
		{
			...
			if (_rigsHandler.ItemInRightHand != null) 
				ItemInRightHand = _rigsHandler.ItemInRightHand;
		...
		{
			_animator.stabilizeFeet = IsRotating;
